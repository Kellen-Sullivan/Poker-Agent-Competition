...
from gym_wrapper import PokerGymEnv
...
class SelfPlayAgent:
    ...
    def load_from_model(self, model):
        """Copy weights from a live training model via save/reload."""
        tmp_path = os.path.join("/tmp", "sp_opponent_tmp")
        model.save(tmp_path)
        self.model = MaskablePPO.load(tmp_path)

    def act(self, observation, env):
        if self.model is None:
            # Fallback to random if no model loaded yet
